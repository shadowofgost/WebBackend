...
def init_logging():
    ...
    pity_info = os.path.join(os.getcwd(), f'{time.strftime("%Y-%m-%d")+"-info"}.log')
    # 为pity添加一个error log文件，主要记录warning和error级别的日志
    pity_error = os.path.join(os.getcwd(), f'{time.strftime("%Y-%m-%d"+"-error")}.log')
    logger.add(
        pity_info,
        enqueue=True,
        rotation="20 MB",
        level="DEBUG",
        filter=make_filter(f'{time.strftime("%Y-%m-%d"+"-info")}'),
    )
    ...
    logger.add(
        ...
        rotation="10 MB",
        level="WARNING",
        filter=make_filter(f'{time.strftime("%Y-%m-%d"+"-error")}'),
    )

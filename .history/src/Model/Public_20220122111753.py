...
class ORM:
    def __init__(self, model : str,session : Session=None,abstract_model :bool=False):
        ...
        if abstract_model == True:
            ...
            self.delete_schema = model_dict[model]["delete_multiple_schema"]
            self.insert_schema = model_dict[model]["insert_multiple_schema"]
            self.update_schema = model_dict[model]["update_multiple_schema"]
            self.select_schema = model_dict[model]["select_multiple_schema"]

    def special_choice_select(self, offset_data = -1, limit_data = -1):

...
class ORM:
    ...
    def id_select(self, id, physical=False):
    ...
    def special_choice_select(
        self, schema, physical:bool=False, offset_data=-1, limit_data=-1
    ):
        if self.abstract_model == True:
    ...
    def insert(
        ...
        if self.abstract_model == True:
            ...
            if multiple == True:
                ...
                try:
                    insert_schema = self.insert_schema(**schema.dict())
                except Exception as e:
                    return error_schema_validation

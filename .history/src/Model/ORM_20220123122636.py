...
def single_table_id_select(model, id: int, session: Session, physical: bool = False):
    """
    ...
    Args:
        model ([type]): [传入的抽象表参数比如Modeluser]
        id (int): [description]
...
def single_table_special_choice_select(
    ...
    """
    single_table_special_choice_select [特殊选择的表]
    ...
    if limit_data == -1 or offset_data == -1:
        ...
        if physical == False:
            ...
            schema_dict["IMark"] = 0
            stmt = (
                select(model).filter_by(**schema_dict).offset(offset_data).limit(limit_data)
            )
        else:
            stmt = (
                select(model)

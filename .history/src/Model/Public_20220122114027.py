...
def multiple_delete(model, multiSchema, session : Session):
    mappings = [
        {'id' : multiSchema.data[i].id,'TimeUpdate' : multiSchema.data[i].TimeUpdate,'IMark' : 1}
...
class ORM:
    def __init__(self, model : str,session : Session=None,abstract_model :bool=False):
        '''
    ...
    def total_select(self, physical=False, offset_data = -1, limit_data = -1):
        if self.abstract_model == True:
            ...
            return model_dict[self.model]['total_select'](self.model,self.session, offset_data, limit_data)
    def special_choice_select(self,schema,physical=False, offset_data = -1, limit_data = -1):
        if self.abstract_model == True:
            return self.model.table_special_choice_select(offset_data, limit_data)
        else:
            return model_dict[self.model]['tabel_special_choice_select'](self.model, schema, self.session, offset_data, limit_data)
    """
    ...
    """
    def insert(self, schema = None, multiple_schema = None,multiple : bool = False, physical:bool = False):
        try:
    ...
    def update(self, schema = None, multiple_schema = None,multiple : bool = False, physical:bool = False):
        try:
    ...
    def delete(self, schema = None, multiple_schema = None,multiple : bool = False, physical:bool = False):
        try:

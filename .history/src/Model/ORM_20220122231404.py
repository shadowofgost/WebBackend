...
def single_table_id_select(model, id:int, session: Session):
...
def single_table_total_select(
    ...
    if limit_data == -1 or offset_data == -1:
        ...
        stmt = select(model).where(model.IMark==0).offset(offset_data).limit(limit_data)

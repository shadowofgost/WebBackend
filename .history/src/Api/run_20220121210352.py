...
from face.WebBackend.src.Api import models, schema, crud
from sqlalchemy.orm import Session
from face.WebBackend.src.Api.database import SessionLocal, engine
import uvicorn
...
def login_for_access_token(db :Session = Depends(get_db), form_data : OAuth2PasswordRequestForm = Depends()):
    user = crud.jwt_authenticate_user(db=db, id=form_data.username, password=form_data.password)
    if not user:

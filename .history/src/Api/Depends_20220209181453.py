...
from ..Services.ModelUserCrud import get_user, model_user_crud_select
...
def get_current_user(
    ...
    try:
        ...
        raise notAuthenticated
    user = get_user(db, id_token)
    return user


def authenticate_user(db: Session, id: str, password: str):
    id_return = int(id)
    user = model_user_crud_select(session=db, id_manager=id_return, service_type=1)
    if not user:
        raise UserNotFound

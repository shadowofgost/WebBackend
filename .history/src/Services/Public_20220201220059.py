...
def select(session: Session,id_manager:int,model:str,service_type:int,schema=None,name=None)->Optional[dict[str,str]]:
    '''
    select [筛选函数的服务提供]
    ...
    model : str
        [description]
    service_type : int
        [根据输入数据判断服务类型,0表示查询所有的数据，1表示查询的是通过id查询数据，2表示通过name查询数据，3表示通过schema查询特定值的数据]

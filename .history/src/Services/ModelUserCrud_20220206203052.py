...
def update(model : str, schema : ModelUserUpdateMultipleGetSchema, session : Session,
           id):
...
def insert(model : str, schema : ModelUserInsertMultipleGetSchema, session : Session,
           id):
    model_instance = ORM(model, session)
    if schema.n == 1:
        try:

...
def update(model : str, schema : ModelMmxDataUpdateMultipleGetSchema, session : Session,
           id):
...
def insert(model : str, schema : ModelMmxDataInsertMultipleGetSchema, session : Session,
           id):
    model_instance = ORM(model, session)
    if schema.n == 1:
        try:

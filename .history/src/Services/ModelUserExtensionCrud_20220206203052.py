...
def update(model : str, schema : ModelUserExtensionUpdateMultipleGetSchema, session : Session,
           id):
    model_instance = ORM(model, session)
    if schema.n == 1:
        try:
            update_table_schema = ModelUserExtensionUpdateSingleTableSchema(IdManager=id)
            result = model_instance.update(update_table_schema)
        except Exception as e:
            return PublicValuesAndSchemas.error_schema_validation
    else:
        try:
            update_table_schema = ModelUserExtensionUpdateMultipleTableSchema(**schema.dict())
            result = model_instance.update(update_table_schema, multiple=True)
        except Exception as e:
            return PublicValuesAndSchemas.error_schema_validation
    return result

def insert(model : str, schema : ModelUserExtensionInsertMultipleGetSchema, session : Session,
           id):
    model_instance = ORM(model, session)
    if schema.n == 1:
        try:
            insert_table_schema = ModelUserExtensionInsertSingleTableSchema(Idmanager=id)
            result = model_instance.update(insert_table_schema)
        except Exception as e:
            return PublicValuesAndSchemas.error_schema_validation
    else:
        try:
            insert_table_schema = ModelUserExtensionInsertMultipleTableSchema(**schema.dict())

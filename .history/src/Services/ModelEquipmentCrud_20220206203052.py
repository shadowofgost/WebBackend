from ..Models.ModelEquipment import (ModelEquipmentUpdateMultipleGetSchema,
...
def update(model : str, schema : ModelEquipmentUpdateMultipleGetSchema, session : Session,
           id):
    ...
    if schema.n == 1:
        ...
        try:
            update_table_schema = ModelEquipmentUpdateMultipleTableSchema(**schema.dict())
            result = model_instance.update(update_table_schema, multiple=True)
        except Exception as e:
            return PublicValuesAndSchemas.error_schema_validation
    return result

def insert(model : str, schema : ModelEquipmentInsertMultipleGetSchema, session : Session,
           id):
    model_instance = ORM(model, session)
    if schema.n == 1:
        try:
        ...
        try:
            insert_table_schema = ModelEquipmentInsertMultipleTableSchema(**schema.dict())

...
class CurriculaGet(BaseModel):
    requires: ModelUserSelectInSingleTableSchema
...
@router.post("/search", response_model=Page[ModelUserSelectOutSingleTableSchema])
...
async def api_model_user_insert(
    schema: ModelUserInsertMultipleGetSchema,
    session: Session = Depends(get_db),
    user: SchemaUserPydantic = Depends(get_current_user),
):
...
async def api_model_user_update(
    schema: ModelUserUpdateMultipleGetSchema,
    session: Session = Depends(get_db),
    user: SchemaUserPydantic = Depends(get_current_user),
):
    schema.n = len(schema.data)
...
async def api_model_user_delete(

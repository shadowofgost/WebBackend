...
router = APIRouter(
    prefix="/model_mmxdata",
    tags=["model_mmxdata"],
)


class CurriculaGet(BaseModel):
    requires: ModelMmxDataSelectInSingleTableSchema
...
@router.get("/", response_model=Page[ModelMmxDataSelectOutSingleTableSchema])
async def api_model_mmxdata_get(
    schema: CurriculaGet,
    Params=Depends(),
    session: Session = Depends(get_db),
    user: ModelMmxDataSelectOutSingleTableSchema = Depends(get_current_user),
):
    model = "ModelMmxData"
...
async def api_model_mmxdata_insert(
    schema: ModelMmxDataInsertMultipleGetSchema,
    session: Session = Depends(get_db),
    user: ModelMmxDataSelectOutSingleTableSchema = Depends(get_current_user),
):
    model = "ModelMmxData"
...
async def api_model_mmxdata_update(
    schema: ModelMmxDataUpdateMultipleGetSchema,
    session: Session = Depends(get_db),
    user: ModelMmxDataSelectOutSingleTableSchema = Depends(get_current_user),
):
    model = "ModelMmxData"
...
async def api_model_mmxdata_delete(
